...
class SkiPassService:
    ...
    # Завдання 3: рівень 2-3.
    def get_skipasses_by_visitor_ids(self, visitor_ids: list[str]) -> list[list[SkiPass]]:
        all_passes = self.db.query(SkiPass).filter(SkiPass.visitor_id.in_(visitor_ids)).all()

        collected_passes = defaultdict(list)
        for passes in all_passes:
            collected_passes[passes.visitor_id].append(passes)

        return [collected_passes[ids] for ids in visitor_ids]
